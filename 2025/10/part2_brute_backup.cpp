...
#include                    }
        ...
        // remove duplicates from new_states using custom comparison
        std::sort(new_states.begin(), new_states.end(), [state_size](const std::array<int, 10>& a, const std::array<int, 10>& b) {
        ...
        new_states.erase(std::unique(new_states.begin(), new_states.end(), [state_size](const std::array<int, 10>& a, const std::array<int, 10>& b) {
            for (int i = 0; i < state_size; i++) {
                if (a[i] != b[i]) return false;
            }
            return true;
...
std::pair<std::array<int, 10>, int> getJoltages(const std::string& str) {
    std::regex pattern(R"(\{([0-9,]+)\})");
    std::smatch match;
    ...
    if (std::regex_search(str, match, pattern)) {
        ...
        std::string num;
        std::istringstream iss(content);
        while (std::getline(iss, num, ',') && size < 10) {
            arr[size] = stoi(num);
            size++;
        }
    }
...
int brute_force(std::pair<std::array<int, 10>, int> &target_state, std::vector<std::vector<int>> &buttons, int state_size) {
    int press_count = 0;
    ...
    while (true) {
        ...
        for (int &button_index : button) {
            if (init_state[button_index] >= target_state.first[button_index] / 2) {
                stop = true;
            }
        }
        if (stop) {
    ...
    while (true)
    {
        ...
        for (std::array<int, 10> &joltages : jolt_states) {
            ...
            for (int &button_index : button) {
                ...
                if (new_state[button_index] > target_state.first[button_index]) {
                    success = false;
                }
            }
            ...
            for (int i = 0; i < state_size; i++) {
                if (new_state[i] != target_state.first[i]) {
                    is_solution = false;
                    break;
                }
            }
